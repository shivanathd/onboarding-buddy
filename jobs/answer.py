...
writes:  nothing
surface: a threaded reply under the mention
...
def run(client, event):
    ...
    made inside a thread, rehearsal check 2b. Both are handled: reply into the
    thread when there is one, otherwise start one under the mention itself.
    """
    if event.get("channel") != policy.CHANNEL_ID:
        client.chat_postMessage(channel=event.get("channel"),
                                thread_ts=event.get("thread_ts") or event.get("ts"),
                                text="I only work in one channel. Ask me there and I will answer.")
    ...
    client.chat_postMessage(channel=policy.CHANNEL_ID, thread_ts=parent, text=reply)
