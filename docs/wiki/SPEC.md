...
2. **This skill's package version** (the `version:` field in `SKILL.md`'s frontmatter): its own release-numbering axis for the skill/plugin itself (bug fixes, secret-scanner
...
- Trust/provenance/attestation fields are adopted, not required. Upstream v0.2 adds
  ...
  an `Attested Computation` type. All of it is optional there, and all of it is optional here too.
...
- The bundle-root `index.md` carries `okf_version` (one of `"0.1"` to `"0.4"`; `scaffold.py`
  still writes `"0.3"` by default, `"0.4"` only with `--trust-signals`) in frontmatter, and only there.
...
The current format version is `0.4`, but `scaffold.py` still emits `0.3` by default (unchanged from before this patch) and only writes `0.4` when explicitly asked
...
declare `0.4` because of the rename; one that keeps declaring `0.1` to `0.3` keeps the old
...
and corrupts the flow sequence, and a colon-space `: ` splits a mapping, so a strict parser
...
- **`sources`** (plural, distinct from the required singular `source`): an optional YAML list
  ...
  the list, and used to key an in-body footnote like `[^warehouse-schema]`) and `resource`
...
| `attester` | mapping `{resource}` pointing at the deterministic, non-LLM checker that compares a receipt against this concept's sanctioned computation |
